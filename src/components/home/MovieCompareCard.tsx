...
interface MovieCompareCardProps {
  kind: ContentKind;
  movie: ThisOrThatItem;
  onPick: (id: string) => void;
  disabled?: boolean;
}

const MovieCompareCard: React.FC<MovieCompareCardProps> = ({ kind, movie, onPick, disabled }) => {
  const isShow = kind === 'show';
  const year = isShow ? movie.first_air_year : movie.release_year;
  // Shows have no single director; use the creators, falling back to networks.
  const subtitle = isShow
    ? (movie.created_by && movie.created_by.length > 0
        ? movie.created_by.join(', ')
        : (movie.networks ?? []).join(', ')) || null
    : movie.director;
  const showMeta =
    isShow &&
    [
    ...
    >
      ...
      >
        ...
        {showMeta && (
          <Typography
            level="body-xs"
            sx={{ color: 'text.tertiary', fontSize: { xs: '0.65rem', sm: '0.75rem' } }}
          >
            {showMeta}
          </Typography>
        )}
